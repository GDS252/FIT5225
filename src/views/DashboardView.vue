  ...
  try {
    ...
    if (files.length > 0) {
      ...
      images.value = files.map(file => {
        ...
        const processedFile = {
          id: file.id || file.fileId || file.filename || Math.random().toString(36),
          filename: file.filename || file.name || file.originalName || 'unknown',
          url: file.url || file.s3_url || file.fileUrl || file.downloadUrl || '',
          thumbnailUrl: file.thumbnailUrl || file.thumbnail || file.url || file.s3_url || file.fileUrl || '',
...
const handleDeleteImage = async (imageId) => {
  try {
    ...
    const imageUrl = image.url || image.s3_url || image.fileUrl || image.src;
