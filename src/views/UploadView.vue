...
const loadRecentUploads = async () => {
  ...
  try {
    ...
    if (response.data && response.data.files) {
      recentUploads.value = response.data.files.map(file => ({
      ...
      recentUploads.value = response.data.slice(0, 6);
    } else {
      recentUploads.value = [];
    }
...
const deleteUpload = async (upload) => {
  try {
    ...
    recentUploads.value = recentUploads.value.filter(item => item.id !== upload.id);
